...
export default {
  ...
  created () {
    ...
    if (this.$store.getters.token) {
      this.$store.dispatch('cart/getCartAsync')
    }
  },
  methods: {
    toggleCheck (goodsId) {
    ...
    changeCount (id, num, skuId) {
      this.$store.dispatch('cart/changeCountAsync', { id, num, skuId })
    },
    async delGoods () {
      if (this.selCount === 0) return
      await this.$store.dispatch('cart/delSelectAsync')
      this.isEdit = false
    },
    goPay () {
      if (this.selCount > 0) {
